class AssignmentParticipationsController < ApplicationController
  ...
  def new
    @form = @assignment_participation.get_form_info({:user => @user, :real_user => @real_user, :controller => self, :form_authenticity_token => form_authenticity_token, :params => params })
  end
